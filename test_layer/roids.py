...
tim = get_timer(callback=display.show, period=33)
'''
...
class Poly():
    ...
    def draw(self, color):
        ...
        Draw the polygon
        '''
        pygfx.polygon(display_drv, self.polygon, self.x, self.y, color, self.angle, 0, 0)
...
def update_missiles():
    ...
    for missile in list(missiles):                      # for each missile
        missile.draw(BLACK)                      # erase old missile
        if missile.counter > 0:                         # if counter > 0 missile is active
            ...
            for roid in list(roids):                    # for each roid
                if missile.collision(roid):             # check if missile collides with roid
                    roid.draw(BLACK)             # erase the roid
            ...
            if missile.counter > 0:                     # if the missile has life left
                missile.draw(WHITE)              # draw missile
...
def explode_ship():
    ...
    ship.counter += 1
    if ship.counter % 2 == 0:
        pygfx.polygon(display_drv, explosion_poly, ship.x, ship.y, BLACK, 0.785398)
        pygfx.polygon(display_drv, explosion_poly, ship.x, ship.y, WHITE)
    else:
        pygfx.polygon(display_drv, explosion_poly, ship.x, ship.y, WHITE, 0.785398)
        pygfx.polygon(display_drv, explosion_poly, ship.x, ship.y, BLACK)
...
def main():
    global ship, roids, ship_alive, missiles
