...
Status: complete. The touchscreen launcher, local app chooser, settings and
IDE dim/wake behavior shipped in the two-board-qualified `modern-v0.15.4`.
The [release record](../../../tests/PHASE6_MODERN_QUALIFICATION.md) documents
that milestone and the subsequent `modern-v0.16.0` qualification baseline.
Use the [focused checklist](../../../tests/MODERN_TOUCHSCREEN_QUALIFICATION.md)
for changes to these behaviors.

## Historical plan and engineering record

The following preserves the original plan and 2026-08-31 engineering checkpoint
from `PROJECT_NOTES.md`. At that checkpoint implementation, host tests and
engineering smoke were complete, but release qualification was still ahead.
References below to future settings, missing qualification, an Elecrow bring-up
target, or the original persistent app-error indicator describe that checkpoint.
Later [button navigation and IDE follow-ups](../../../boards/lilygo_t_display_s3/DEVELOPMENT_RESULTS.md)
record non-touch input and revised error notices; they have their own evidence.
...
preserved in [CST226 upstream recommendations](../active/cst226-upstream.md).
...
At the 2026-08-31 checkpoint the feature was not release-qualified. The following
sequence was still required; the later release record above documents completion:
