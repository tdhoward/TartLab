...
from board_catalog import default_board, selector_source, select_board  # noqa: E402
...
MODERN_SELECTOR = selector_source(PROFILE_BOARD)
BACKUP_PATHS = (
...
def _migrated_repos(backup: Path, version: str,
                    board: dict[str, Any] = PROFILE_BOARD
    ...
    tartlab.update({
        ...
        "board_id": board["id"],
        "firmware_sha256": board["firmware"]["sha256"],
    })
    repos["list"] = entries
...
def prepare_image(release: Path, backup: Path, image: Path, *,
                  version: str, mode: str, backup_identifier: str,
                  board: dict[str, Any] = PROFILE_BOARD) -> None:
    ...
    device = image / "device"
    if mode == "migrate":
        if board["id"] != PROFILE_BOARD["id"]:
            raise ValueError(
                "direct legacy migration is not approved for this board")
        _validate_migration_source(backup)
        _copy_tree_if_present(backup / "device", device)
    device.mkdir(parents=True, exist_ok=True)
    (device / "hdwconfig.py").write_text(
        selector_source(board), encoding="utf-8", newline="\n")
    write_json(device / "board.json", {
        "schema": 1,
        "board_id": board["id"],
    })
    ...
    repos, previous = _migrated_repos(backup, version, board)
...
def _release_identity(release: Path, board: dict[str, Any] = PROFILE_BOARD
                      ) -> tuple[dict[str, Any], str]:
    manifest = _read_object(release / "modern-manifest.json")
    check_modern_release(
        release, PROFILE, board["firmware"]["sha256"], board_id=board["id"])
...
def provision(release: Path, workspace: Path, mode: str, transport: Any, *,
    ...
    if mode == "migrate" and board_id != PROFILE_BOARD["id"]:
        raise ValueError("direct legacy migration is not approved for this board")
    release = release.resolve()
    manifest, release_identifier = _release_identity(release, board)
    version = manifest["version"]
    published = manifest["published_assets"]
    firmwares = published.get("firmwares")
    if isinstance(firmwares, dict):
        firmware_record = firmwares.get(board_id)
    elif board_id == PROFILE_BOARD["id"]:
        firmware_record = published.get("firmware")
    else:
        firmware_record = None
    if not isinstance(firmware_record, dict):
        raise ValueError("release has no firmware for the selected board")
    ...
    if _stage_index(stage) < _stage_index("awaiting_health"):
        ...
        prepare_image(
            ...
            backup_identifier=journal["backup_identifier"], board=board)
        transport.install(
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.mode == "migrate" and board["id"] != PROFILE_BOARD["id"]:
        raise ValueError("direct legacy migration is not approved for this board")
    manifest, release_identifier = _release_identity(
        args.release.resolve(), board)
    if not args.execute:
        print(json.dumps({
